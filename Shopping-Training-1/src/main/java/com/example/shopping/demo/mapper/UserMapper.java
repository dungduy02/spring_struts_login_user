...
//	List<User> findAll();
	...
	@Select("select * from user where (`name` like concat('%',#{fullname}, '%') AND email like concat('%', #{email}, '%') AND `group` like concat('%', #{groups}, '%') AND is_active like concat('%', #{active}, '%') AND is_delete = 0) ")
	List<User> getByUsers(@Param("fullname") String name,@Param("email") String email, @Param("groups") String groups, @Param("active") String active);
	...
	@Select("UPDATE `user` SET usename = #{name}, `name` = #{name}, email = #{email}, password = #{password}, `group` = #{group}, is_active = 1, create_day = CURRENT_TIMESTAMP(), is_delete = 0 WHERE  id = #{id}")
	void editUser(@Param("name") String name,@Param("email") String email, @Param("password") String password, @Param("group") String group, @Param("id") Long id);
